    ...
    <section className={styles.panel} id="postulantes">
      <div className={styles.heading}>
        <div>
          ...
          <p>
            Consulta la información importada desde el archivo institucional.
          </p>
        </div>
        {canWrite && (
      ...
      {error ? (
        <div className={styles.errorBox} role="alert">
          ...
          <span>
            {error}. Ejecuta <code>pnpm dev</code> para iniciar ambas
            aplicaciones.
          </span>
        </div>
      ) : (
        <>
              ...
              <tbody>
                {isLoading ? (
                  Array.from({ length: 7 }, (_, i) => (
                    <tr className={styles.skeleton} key={i} aria-hidden="true">
                      <td>
                        <i className={styles.wide} />
                      </td>
                      <td>
                        <i />
                      </td>
                    ...
                    <tr key={a.id}>
                      <td data-label="Postulante">
                        <div className={styles.person}>
                          <span>{initials(a.nombres, a.apellidos)}</span>
                          <div>
                            <strong>
                              {a.nombres} {a.apellidos}
                            </strong>
                            <small>
                              {a.facultad || "Facultad no registrada"}
                            </small>
                          </div>
                        </div>
                      </td>
                      <td data-label="DNI" className={styles.mono}>
                        {a.dni}
                      </td>
                      <td data-label="Convocatoria">{a.convocatoria || "—"}</td>
                      <td data-label="Carrera">{a.carrera || "—"}</td>
                      <td data-label="Puntaje" className={styles.score}>
                        {a.puntaje ?? "—"}
                      </td>
          ...
          <footer className={styles.pagination}>
            <span>
              ...
              {formatNumber(response?.meta.total)}
            </span>
            <div>
              <button
                type="button"
                onClick={() => onPage((c) => c - 1)}
                disabled={isLoading || page <= 1}
              >
                Anterior
              </button>
              <b>
                Página {page} de {response?.meta.totalPages || 1}
              </b>
              <button
                type="button"
                onClick={() => onPage((c) => c + 1)}
