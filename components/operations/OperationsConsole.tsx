...
export type Screen = "postulante" | "inscripcion" | "ordenes" | "tesoreria" | "resultados" | "catalogos" | "usuarios" | "auditoria";
...
export function OperationsConsole({ token, role }: { token: string; role?: string }) {
  ...
  async function refresh() {
    ...
    try {
      const [catalogos, inscripciones, ordenes, cajas, pagos, resultados, dashboard] = await Promise.all(paths.map((p) => api<unknown>(p)));
      let auditoria: Row[] = [];
      let usuarios: Row[] = [];
      try { auditoria = await api<Row[]>("/auditoria"); } catch { /* Solo administradores. */ }
      try { usuarios = await api<Row[]>("/usuarios"); } catch { /* Solo administradores. */ }
      setResources({
        ...
        auditoria, usuarios, dashboard: dashboard as Row,
      });
    } catch (error) {
  ...
  return (
    <section className={styles.panel} id="operaciones">
      <div className={styles.header}>
        ...
        <span className={styles.sync}>{loading ? "Sincronizando..." : "Datos actualizados"}</span>
      </div>
      <div className={styles.tabs} role="tablist" aria-label="Módulos del sistema">
        {visibleScreens.map((item) => (
          <button key={item.id} className={screen === item.id ? styles.active : ""} onClick={() => { setScreen(item.id); setNotice(undefined); }} role="tab" aria-selected={screen === item.id}>
            <span>{item.step}</span>{item.label}
          </button>
        ))}
      </div>
      {notice && <div className={`${styles.notice} ${notice.tone === "success" ? styles.success : styles.error}`} role="status">{notice.text}</div>}
      <div className={styles.screen}>
        {screen === "postulante" && <ApplicantForm submit={submit} />}
        {screen === "inscripcion" && <EnrollmentSection resources={resources} api={api} submit={submit} />}
        {screen === "ordenes" && <OrdersSection orders={resources.ordenes} />}
        {screen === "tesoreria" && <TreasurySection resources={resources} api={api} submit={submit} />}
        {screen === "resultados" && <ResultsSection resources={resources} submit={submit} />}
        {screen === "catalogos" && <CatalogsSection catalogs={resources.catalogos} submit={submit} />}
        {screen === "usuarios" && <UsersSection rows={resources.usuarios} submit={submit} />}
