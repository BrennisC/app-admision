...
export function EnrollmentSection({ resources, api, submit }: { resources: Resources; api: Api; submit: Submit }) {
  ...
  const filtered = resources.inscripciones.filter((row) => {
    ...
    return `${text(row.id_inscripcion)} ${text(row.estado)} ${text(row.modalidad)}`.toLowerCase().includes(filter.toLowerCase());
  });
  ...
  return (
    <ScreenLayout title="Crear inscripción" description="Localiza al postulante y genera una orden de pago automáticamente." aside="Una persona solo puede mantener una inscripción activa por convocatoria.">
      <div className={styles.lookup}>
        <Field label="DNI del postulante"><input value={dni} onChange={(e) => setDni(e.target.value)} maxLength={8} /></Field>
        <button type="button" className="secondary-button" onClick={findApplicant} disabled={dni.length !== 8}>Buscar persona</button>
      </div>
      {lookupError && <p className={styles.error}>{lookupError}</p>}
      {applicant && <div className={styles.found}><span>{String(applicant.nombres)[0]}{String(applicant.apellidos)[0]}</span><div><strong>{text(applicant.nombres)} {text(applicant.apellidos)}</strong><small>DNI {text(applicant.dni)} · ID {text(applicant.id)}</small></div></div>}
      <form className={styles.form} id="inscripciones" onSubmit={async (event) => {
        event.preventDefault();
        if (!applicant) return;
        const form = new FormData(event.currentTarget);
        const ok = await submit("/inscripciones", {
      ...
      <h3 className={styles.subtitle}>Inscripciones registradas ({filtered.length})</h3>
      <div className={styles.lookup}><Field label="Filtrar por estado o modalidad"><input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Ej: PAGADO, PENDIENTE_PAGO…" /></Field></div>
      <DataTable
        rows={filtered}
        columns={["id_inscripcion", "id_postulante", "id_carrera", "modalidad", "estado"]}
        action={(row) => row.estado === "PAGADO" ? { label: "Confirmar", onClick: () => void submit(`/inscripciones/${text(row.id_inscripcion)}/confirmacion`, {}, "Inscripción confirmada") } : undefined}
      />
    </ScreenLayout>
  );
}
