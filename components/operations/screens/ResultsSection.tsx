...
export function ResultsSection({ resources, submit }: { resources: Resources; submit: Submit }) {
  const confirmable = resources.inscripciones.filter((row) => row.estado === "PAGADO");
  const confirmed = resources.inscripciones.filter(
    (row) => row.estado === "CONFIRMADO" && !resources.resultados.some((r) => number(r.id_inscripcion) === number(row.id_inscripcion)),
  );
  ...
  return (
    <ScreenLayout title="Confirmación y resultados" description="Confirma inscripciones pagadas y registra su resultado académico." aside="No se admite un resultado sobre una inscripción pendiente o no confirmada.">
      <div className={styles.confirmList} id="resultados">
        {confirmable.slice(0, 6).map((row) => (
          <div key={text(row.id_inscripcion)}>
            ...
            <button onClick={() => void submit(`/inscripciones/${text(row.id_inscripcion)}/confirmacion`, {}, "Inscripción confirmada")}>Confirmar</button>
          </div>
        ))}
        {!confirmable.length && <p>No hay inscripciones pagadas pendientes de confirmación.</p>}
      </div>
      <form className={styles.form} onSubmit={async (event) => {
        event.preventDefault();
        const form = new FormData(event.currentTarget);
        await submit("/resultados", {
      ...
      <DataTable rows={resources.resultados.slice(0, 8)} columns={["id_inscripcion", "puntaje", "puesto", "condicion"]} />
    </ScreenLayout>
  );
}
