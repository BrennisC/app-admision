...
export function TreasurySection({ resources, api, submit }: { resources: Resources; api: Api; submit: Submit }) {
  ...
  return (
    <ScreenLayout
      ...
      aside={openCash ? `Caja ${text(openCash.id_caja)} abierta por ${text(openCash.usuario)} · saldo inicial S/ ${Number(openCash.saldo_inicial).toFixed(2)}.` : "Debes abrir una caja antes de cobrar. Una caja cerrada no acepta movimientos."}
    >
      {!openCash ? (
        <form className={styles.compact} id="tesoreria" onSubmit={async (event) => {
          event.preventDefault();
          const form = new FormData(event.currentTarget);
          await submit("/cajas/apertura", { saldoInicial: number(form.get("saldoInicial")) }, "Caja abierta correctamente");
        }}>
          <Field label="Saldo inicial"><input name="saldoInicial" type="number" min="0" step="0.01" defaultValue="0" required /></Field>
          <button className="secondary-button">Abrir caja</button>
        </form>
      ) : (
        <form className={styles.compact} onSubmit={async (event) => {
          event.preventDefault();
          const form = new FormData(event.currentTarget);
          await submit(`/cajas/${text(openCash.id_caja)}/cierre`, { saldoFinal: number(form.get("saldoFinal")) }, "Caja cerrada y arqueada");
        }}>
          <Field label="Saldo final declarado (arqueo)"><input name="saldoFinal" type="number" min="0" step="0.01" required /></Field>
      ...
      <form className={styles.form} onSubmit={async (event) => {
        event.preventDefault();
        const form = new FormData(event.currentTarget);
        const order = pendingOrders.find((row) => number(row.id_orden) === number(form.get("idOrden")));
      ...
      <h3 className={styles.subtitle}>Últimos pagos · total S/ {totalPagos.toFixed(2)}</h3>
      <div className={styles.receipts}>
        {resources.pagos.slice(0, 8).map((row) => (
          <div key={text(row.id_pago)}>
            <span><strong>{text(row.codigo_pago)}</strong><small>S/ {Number(row.monto).toFixed(2)} · {text(row.metodo_pago)} · {text(row.voucher) || "sin voucher"}</small></span>
            <button onClick={() => void printReceipt(number(row.id_pago), api)}>Ver comprobante</button>
          </div>
        ))}
        {!resources.pagos.length && <div className={styles.empty}>Todavía no hay pagos.</div>}
      </div>
